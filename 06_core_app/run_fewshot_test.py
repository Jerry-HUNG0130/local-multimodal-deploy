#!/usr/bin/env python3
"""
Round 9: Few-Shot Prompt Engineering 測試
在 R8a 最佳配置基礎上，加入 3 個 few-shot 範例引導 Llama3 的回答風格。
配置：v3 + sentence rewrite + HyDE + BM25 hybrid + reranker + few-shot
"""
import os
import sys
import json
...
def run_test():
    db_version = "v3"
    run_label = "R9_dbv3_k3_rw-sentence_hyde_hybrid-bm25_reranker_fewshot"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    print("=" * 60)
    print(f"  🚀 Round 9: Few-Shot Prompt Engineering 測試")
    print(f"  配置: v3 + sentence rewrite + HyDE + BM25 + reranker + few-shot")
    print("=" * 60)
    ...
    db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "04_knowledge_engine", "vector_db_v3")
