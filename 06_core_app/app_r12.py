"""
RAG 伺服器 — R12 配置（Qwen2.5-7B-Instruct，品質最佳 ~6s/題）
配置：v3 + sentence rewrite + HyDE + BM25 Hybrid (RRF) + Reranker + Qwen2.5
適合：品質展示、即時互動
"""
import os
import time
...
# 初始化元件
# ============================================================
print("📦 正在載入 RAG 引擎 (R12 配置 — Qwen2.5-7B-Instruct)...")
...
llm = Ollama(model="qwen2.5:7b-instruct-q8_0", temperature=0.0)
...
print("✅ R12 RAG 引擎就緒（Qwen2.5-7B-Instruct）\n")
...
def hyde_generate(question):
    """HyDE: 產生假設性規章回答，用於改善 Vector Search 語義匹配"""
    prompt = (
...
def hybrid_retrieve(bm25_query, vector_query=None, fetch_k=9):
    """Vector + BM25 → RRF。vector_query 可用 HyDE 假答案"""
    vec_q = vector_query if vector_query else bm25_query

    # Vector search（用 HyDE 假答案）
    ...
    # BM25 search（用 sentence rewrite 查詢）
...
async def chat_with_rag(req: ChatRequest):
    ...
    # Step 3: Hybrid Retrieve (Vector 用 HyDE + BM25 用 sentence rewrite → RRF)
...
if __name__ == "__main__":
    print("🚀 啟動 RAG 伺服器（R12 配置 — Qwen2.5 品質最佳 ~6s/題）")
