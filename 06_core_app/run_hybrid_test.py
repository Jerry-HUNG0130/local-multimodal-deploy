#!/usr/bin/env python3
"""
Round 6: Hybrid Search 測試
在 Round 5 最佳配置 (v2 + bge-m3 + k=3 + sentence rewrite + reranker) 基礎上
加入 BM25 Hybrid Search

檢索策略：
  sentence rewrite → Vector search (top 9) + BM25 search (top 9)
  → RRF 合併 → bge-reranker-v2-m3 重排 → top 3 → LLM
"""
import os
import sys
import json
...
def main():
    run_label = "R6_dbv2_k3_rw-sentence_hybrid-bm25_reranker"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    print("=" * 60)
    print("  🚀 Round 6: Hybrid Search (Vector + BM25) 測試")
    print("  配置: v2 + bge-m3 + sentence rewrite + BM25 + reranker")
    print("  策略: sentence rewrite → Vector(9) + BM25(9) → RRF → reranker(3) → LLM")
    print("=" * 60)
