#!/usr/bin/env python3
"""
Round 12c: Qwen2.5 + Prompt v2 優化
R12 配置 + 強化 prompt（排除條款提示 + 用語連結 + 繁體強制）
"""
import os
import sys
import json
...
def run_test():
    run_label = "R12c_dbv3_k3_rw-sentence_hyde_hybrid-bm25_reranker_prompt-v2_qwen25"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    db_version = "v3"
    llm_model = "qwen2.5:7b-instruct-q8_0"

    print("=" * 60)
    print(f"  Round 12c: Qwen2.5 + Prompt v2 優化")
    print(f"  配置: {db_version} + sentence + HyDE + BM25 + reranker + prompt-v2")
