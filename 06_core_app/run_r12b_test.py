#!/usr/bin/env python3
"""
Round 12b: Qwen2.5-7B-Instruct + Query Decomposition
R12 最佳配置 + decompose（複合問題拆分檢索）
驗證 Qwen2.5 的 32K context + 強閱讀理解力能否吃下 decompose 帶來的更多 context
"""
import os
import sys
import json
...
def run_test():
    run_label = "R12b_dbv3_k3_rw-sentence_hyde_hybrid-bm25_reranker_decompose_qwen25"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    db_version = "v3"
    llm_model = "qwen2.5:7b-instruct-q8_0"

    print("=" * 60)
    print(f"  Round 12b: Qwen2.5 + Query Decomposition")
    print(f"  配置: {db_version} + sentence + HyDE + BM25 + reranker + decompose")
